...
function bindForm() {
  document.getElementById('add-project-form')?.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      // Используем постоянную ссылку через GitHub API (не raw, т.к. там токен протухает)
      // Для публичных репозиториев это работает без авторизации
      const branch = ghCfg.branch || 'main';
      const encodedPath = path.split('/').map(encodeURIComponent).join('/');
      const downloadUrl = `https://raw.githubusercontent.com/${ghCfg.owner}/${ghCfg.repo}/${ghCfg.branch || 'main'}/${path}`;
