...
import { isLoggedIn, getUser, getProfile, isAdmin } from '../auth.js';
  ...
  <div class="project-detail-layout">
    ...
    <div>
      <div class="project-detail-sidebar-card">
        <!-- Download -->
        ${p.downloadUrl ? `
        <button class="btn btn--primary btn--full" id="download-btn" style="margin-bottom:1rem;">
          ↓ Скачать ZIP
        </button>` : `<p style="text-align:center;color:var(--text-muted);font-size:.85rem;margin-bottom:1rem;">Архив недоступен</p>`}
...
function bindProjectEvents() {
  ...
  document.getElementById('download-btn')?.addEventListener('click', async () => {
    ...
    try {
      ...
      // Получаем токен из настроек (только если репозиторий приватный)
      const settingsSnap = await getDoc(doc(db, 'settings', 'app'));
      const ghCfg = settingsSnap.exists() ? (settingsSnap.data().github || {}) : {};

      // Пробуем скачать файл
      const url = _project.downloadUrl;
      let response;

      if (ghCfg.token) {
        // Конвертируем download URL в API URL для приватных репо
        ...
        if (match) {
          ...
          if (metaRes.ok) {
            ...
            // Скачиваем blob напрямую через download_url с токеном
            response = await fetch(meta.download_url, {
      ...
      // Fallback: прямая ссылка (для публичных репо)
      ...
      const filename = url.split('/').pop().split('?')[0] || 'project.zip';
...
async function toggleFav() {
  ...
  try {
    ...
    // Обновляем _profile локально
    const prof = getProfile();
    if (prof) {
      if (isActive) {
        prof.favorites = (prof.favorites || []).filter(x => x !== _projectId);
      } else {
        prof.favorites = [...(prof.favorites || []), _projectId];
      }
    }
  } catch (e) {
    _project._isFav = isActive;
