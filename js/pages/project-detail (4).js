...
// Конвертирует любую GitHub ссылку в raw.githubusercontent.com без токена
function toRawUrl(url) {
  if (!url) return url;
  // Убираем ?token=... если есть
  url = url.split('?')[0];
  // github.com/owner/repo/raw/refs/heads/branch/path -> raw.githubusercontent.com/owner/repo/branch/path
  url = url.replace(
    /https:\/\/github\.com\/([^/]+)\/([^/]+)\/raw\/refs\/heads\/([^/]+)\/(.*)/,
    'https://raw.githubusercontent.com/$1/$2/$3/$4'
  );
  // raw.githubusercontent.com уже правильный — оставляем как есть
  return url;
}
  ...
  <div class="project-detail-layout">
    ...
    <div>
      <div class="project-detail-sidebar-card">
        <!-- Download -->
        ${p.downloadUrl ? `
        <a href="${escHtml(toRawUrl(p.downloadUrl))}" target="_blank" rel="noopener"
           class="btn btn--primary btn--full" id="download-btn" style="margin-bottom:1rem;">
          ↓ Скачать ZIP
